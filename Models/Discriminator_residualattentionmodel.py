...
class ResidualAttentionModel_448input(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias = False),# change to 1
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        )
        self.mpool1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        ...
        self.fc = nn.Linear(2048,4)# change to 4
...
class ResidualAttentionModel_92(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Linear(2048,4)# change to 1
...
class ResidualAttentionModel_92_32input(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=5, stride=1, padding=2, bias=False),# change to 1
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True)
        )  # 32*32
        ...
        self.fc = nn.Linear(1024,4)# change to 4
...
class ResidualAttentionModel_92_32input_update(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1, bias=False),# change to 1
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True)
        )  # 32*32
        # self.mpool1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)  # 16*16
